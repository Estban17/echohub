...
const CulturalExplorer = () => {
  ...
  return (
    <div className="bg-[#181818] p-6 rounded-2xl border border-gray-800 h-full flex flex-col items-center">
      ...
      {/* Cambiamos min-h-[160px] por min-h-40 para quitar la advertencia */}
      <div className="w-full mt-2 p-4 bg-black/40 rounded-xl border border-white/5 min-h-40 flex flex-col justify-center">
        {isSpinning ? (
          ...
          <div className="animate-in fade-in zoom-in duration-500 text-left">
            <div className="flex justify-between items-center mb-2">
               ...
               <span className="text-[10px] text-gray-400 font-mono italic">#{selected.id}</span>
            </div>
      ...
      <button 
        ...
        className={`w-full mt-6 py-4 font-black rounded-lg transition-all active:scale-95 cursor-pointer flex items-center justify-center gap-2 ${
          isSpinning ? 'bg-gray-700 text-gray-400' : 'bg-white text-black hover:bg-spotify-green'
        }`}
      >
        <span>{isSpinning ? 'BUSCANDO...' : 'GIRAR GLOBO TERRAQUEO'}</span>
      </button>
    </div>
  );
};
